...
class RewardEncoder(nn.Module):
    def __init__(self, num_rws=7, embed_dim=64, mask_value=-100):
        ...
        self.fcs = nn.ModuleList(
            [torch.nn.Linear(1, embed_dim) for _ in range(num_rws)]
        )
    ...
    def forward(self, rs):
        ...
        bs, length, _ = rs.shape
        attn_mask = rs == self.mask_value
        if (attn_mask + (rs == 0)).all():
            return torch.zeros(bs, length, self.embed_dim).to(rs.device)

        rs = rs.unsqueeze(2)
        rs = torch.stack(
            [self.fcs[i](rs[:, :, :, i]) for i in range(self.num_rws)], dim=-2
        )
        rs = self.activation(rs)

        rs = rs.view(-1, self.num_rws, self.embed_dim)
        attn_mask = attn_mask.view(-1, self.num_rws).unsqueeze(1)
        q = self.query.expand(rs.shape[0], 1, self.embed_dim)

        out, att_map = self.attention(q, rs, rs, attn_mask=attn_mask)

        return out.view(bs, length, -1), att_map
...
class DecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.predict_return = torch.nn.Linear(hidden_size, self.return_dim)
