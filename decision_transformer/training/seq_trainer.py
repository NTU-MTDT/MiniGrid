...
class SequenceTrainer(Trainer):
    ...
    def train_step(self):
        ...
        reward_preds = reward_preds.reshape(-1, return_dim)[attention_mask.reshape(-1) > 0]
        ...
        with torch.no_grad():
            self.diagnostics["training/action_error"] = np.mean(
                np.argmax(action_preds.detach().cpu().numpy(), axis=1) != np.argmax(action_target.detach().cpu().numpy(), axis=1)
            )
